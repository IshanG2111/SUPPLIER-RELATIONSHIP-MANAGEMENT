...
const workflow = [
  { label: 'RFQ intake', value: 82, color: 'bg-blue-500' },
  { label: 'Bid evaluation', value: 64, color: 'bg-violet-500' },
  { label: 'PO conversion', value: 73, color: 'bg-emerald-500' },
  { label: 'Compliance review', value: 91, color: 'bg-amber-500' },
];
...
export function AdminDashboard() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      <motion.section
        variants={item}
        className="relative overflow-hidden rounded-lg border border-slate-200 bg-slate-950 text-white shadow-[0_24px_80px_rgba(15,23,42,0.18)] dark:border-slate-800"
      >
        <div className="absolute inset-0 bg-[linear-gradient(120deg,rgba(37,99,235,0.28),transparent_38%,rgba(20,184,166,0.2)_72%,transparent)]" />
        ...
        <div className="relative grid gap-6 p-6 lg:grid-cols-[1.35fr_0.65fr] lg:p-7">
          <div>
            <motion.span
              ...
              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs font-bold text-cyan-100"
            >
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link to="/admin/rfqs" className="inline-flex items-center gap-2 rounded-md bg-white px-4 py-2.5 text-sm font-bold text-slate-950 transition hover:bg-cyan-50">
                Create RFQ <ArrowRight className="h-4 w-4" />
              </Link>
              <Link to="/admin/analytics" className="inline-flex items-center gap-2 rounded-md border border-white/15 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-white/10">
                View analytics
              </Link>
            </div>
          </div>
            ...
            {dashboardWorkflow.map((step, index) => (
              ...
              >
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-black text-cyan-100">{step.value}%</span>
                </div>
