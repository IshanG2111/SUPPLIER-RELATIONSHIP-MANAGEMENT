...
├── user/           # Supplier portal and profile management
├── auth/           # Login, registration, and session handling
├── includes/       # Shared layouts (header, footer) and db connection
├── assets/         # CSS, JS, Images, and user uploads
├── database/       # SQL schema and seed data
├── pages/          # Standalone application pages
└── index.php       # Landing/routing page
...
1. **Environment:** Install and start XAMPP (ensure Apache and MySQL are running).
2. **Clone Repository:** Place the project folder (`srm-project`) inside your `xampp/htdocs/` directory.
3. **Database Configuration:**
* Open `http://localhost/phpmyadmin`.
* Create a new database named `srm_system`.
* Import the provided SQL file from the `database/` folder.


4. **Connect:** Verify the credentials in `includes/db.php` match your local MySQL setup (default is usually `root` with a blank password).
5. **Run:** Open `http://localhost/srm-project` in your browser.
